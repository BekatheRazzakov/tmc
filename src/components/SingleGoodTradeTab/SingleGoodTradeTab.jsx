...
import {
  Autocomplete, Box, FormControl, Snackbar, TextField,
} from '@mui/material';
...
const SingleGoodTrageTab = ({ goodId }) => {
  ...
  const handleChange = (e) => {
    ...
    const selectedUser = users.filter((user) => user?.id === value)[0];
  ...
  return (
    <div
      className='single-good-delete-tab'
      ...
      >
        ...
        >
          <Autocomplete
            ...
            onChange={(e, value) => handleChange({ target: { value: value?.id } })}
            renderInput={(params) => (
